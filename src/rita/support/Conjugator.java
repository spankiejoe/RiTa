 ...
 * An example: <pre>
    ...
    String c = rc.conjugate("announce");</pre>    
    <br>
...
{    
  ...
  private void createRuleMap() {
    ruleMap = new HashMap();
    ruleMap.put(PAST_TENSE_RULE, new Rule("PAST_TENSE", DEFAULT_PAST_RULE, PAST_TENSE_RULES));
    ruleMap.put(PRESENT_TENSE_RULE, new Rule("PRESENT_TENSE", DEFAULT_PRESENT_TENSE, PRESENT_TENSE_RULES));
    ruleMap.put(PAST_PARTICIPLE_RULE, new Rule("PAST_PARTICIPLE", DEFAULT_PP_RULE, PAST_PARTICIPLE_RULES));
    ruleMap.put(PRESENT_PARTICIPLE_RULE, new Rule("ING_FORM", DEFAULT_ING_RULE, ING_FORM_RULES));
  }
   ...
   * forms for different person and number values (<I>am</I>, <I>are</I>
   * etc).
   */
  ...
  private String apply(Rule rule, String verb) 
  {
    ...
    if (dbug)System.out.println("RULE: "+rule);
    ...
    for (int i = 0; i < _rules.length; i++) {
      ...
      if (dbug)System.out.println("  '"+baseForm+"'\n  checking: "+currentRule);
      
      if (currentRule.applies(baseForm)) {
        
        if (dbug)System.out.println("  HIT: "+currentRule);        
    ...
    if ((result == null) && (_defaultRule != null)) 
    {
      ...
      if (dbug)System.out.println("  DEFAULT: "+_defaultRule);        

      result = _defaultRule.fire(baseForm);
    }

    if (dbug)System.out.println("  RESULT: "+result);        
  ...
  // VERB_FORMS ---------------------------------------
	...
	private static RegexRule[] PAST_TENSE_RULES = new RegexRule[] {
			...
			new RegexRule("^" + VERBAL_PREFIX + "?lie$",
					2, "ay", 0),
			new RegexRule("^" + VERBAL_PREFIX
					+ "?(weep|sleep|sweep|creep|keep)$", 2, "pt", 0),
			new RegexRule("^" + VERBAL_PREFIX
					+ "?(sell|tell)$", 3, "old", 0),
			new RegexRule("^" + VERBAL_PREFIX + "?do$",
					1, "id", 0),
			new RegexRule("^" + VERBAL_PREFIX + "?dig$",
					2, "ug", 0),
			new RegexRule("^behave$", 0, "d", 0),
			new RegexRule("^(have)$", 2, "d", 0),
			...
			new RegexRule("^" + VERBAL_PREFIX + "?run$",
					2, "an", 0),
	...
	private static RegexRule[] PRESENT_TENSE_RULES = new RegexRule[] {
			...
			new RegexRule("^ko$", 0, "'s", 0),
			new RegexRule("[osz]$", 0, "es", 1),
			new RegexRule("^have$", 2, "s", 0),
			new RegexRule(CONS + "y$", 1, "ies", 1),
			new RegexRule("^be$", 2, "is"),
...
/*
 ...
 static List AUXILIARIES = Arrays.asList(new String[] { "do", "have", "be" });
  ...
  static List VERB_CONS_DOUBLING = Arrays.asList(new String[] {
      ...
      "wit", "woodchop", "woodcut", "wor", "worship", "wrap", "wiretap",
      "yen", "yak", "yap", "yarnspin", "yip", "yodel", "zag", "zap",
      "zig", "zig-zag", "zigzag", "zip", "ztrip", "hand-bag", "hocus",
      "hocus-pocus" });

  static List MODALS = Arrays.asList(new String[] { "shall",
 ...
 /*
   Sets the form for this conjugation. 
   REPLACE!

  public void setForm(int theForm) {
    form = theForm;
    // can't have PAST or FUTURE with gerunds or infinitives
    if ((form == GERUND) || (form == INFINITIVE)
        || (form == BARE_INFINITIVE))
      tense = PRESENT_TENSE;
  } 
*/
  ...
  public Conjugator handleArgs(Map args)
  {
    ...
    this.progressive = (aprogressive!=null) ? (Boolean)(aprogressive) : false;

    Object ainterrogative = args.get("interrogative");
    this.interrogative = (ainterrogative!=null) ? (Boolean)(ainterrogative) : false;
  ...
  public void setNumber(String numberString) {
    ...
    throw new RiTaException("Unexpected value for number: "+
      numberString+", must be one of "+Arrays.asList(numbers));
  }
  ...
  public void setPerson(String personString) {
    ...
    throw new RiTaException("Unexpected value for person: "+
      personString+", must be one of "+Arrays.asList(persons));
  }
  ...
  public void setTense(String tenseString) {
    ...
    throw new RiTaException("Unexpected value for tense: "+
      tenseString+", must be one of "+Arrays.asList(tenses));
  }
  ...
  class CONST {
    ...
    public boolean equals(int i) { return val == i; }
  ...
  /** A convenience method to set number, person, & tense, then conjugate, all in one call */
  public String conjugate(int numberConstant, int personConstant, int tenseConstant, String verb) {
  ...
  public String[] conjugateAll(String verb) {
    ...
    for (int i = 0; i < tenses.length; i++) {
      ...
      for (int j = 0; j < numbers.length; j++) {
        setNumber(numbers[j].val);
        for (int k = 0; k < persons.length; k++) {
          setPerson(persons[k].val);        
          for (int l = 0; l < 2; l++) {
            setPassive(l==0 ? true : false);
            for (int m = 0; m < 2; m++) {
              setProgressive(m==0 ? true : false);
              for (int n = 0; n < 2; n++) {
                setPerfect(n==0 ? true : false);
                tmp.add(conjugate());
                if (DBUG) System.out.println(this);
              }              
            }
          }          
        }
