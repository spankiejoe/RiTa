...
public class RiStringTest implements Constants
{
  ...
  { 
    ...
    equal(features.get(PHONEMES), "dh-ax l-ae-g-ih-n d-r-ae-g-ax-n");
    equal(features.get(SYLLABLES),"dh-ax l-ae/g-ih-n d-r-ae-g/ax-n");
  ...
  public void testIndexOf()
  {
   ...
   /*  rs = new
     ...
     equal(result, 40);*/ //TODO 2 parameters not implemeted yet
...
/*     rs = new RiString("Start at first character. Start at last character.");
  ...
  public void testReplaceFirstRegex()
  {
     ...
     rs = new RiString("Watch out for the rock!"); 
     rs.replaceFirst("r?or?", "a"); equal(rs.text(), "Watch aut for the rock!");
     
     rs = new RiString("The rain in SPAIN stays mainly in the plain");
     rs.replaceFirst("in", ""); equal(rs.text(),"The ra in SPAIN stays mainly in the plain");
     
     // rs = new RiString("Who are you?"); rs.replaceFirst("?", "?!"); // illegal regex
     rs = new RiString("Who are you?"); rs.replaceFirst("\\?", "?!"); 
     equal(rs.text(), "Who are you?!");
  }
  ...
  public void testReplaceWordAt()
  {
    ...
    // equal(rs.text(), "Who are?"); // strange case, not sure (in RiTaJS)
  ...
  public void testSplit()
  {
    ...
    deepEqual(result, answer);
     ...
     rs = new RiString("Who are you?"); 
     result = rs.split("", 3); 
     answer = new RiString[] { RiString("W"), RiString("h"), RiString("o") }; 
     deepEqual(result, answer);
     ...
     result = rs.split("", 100); 
     answer = new RiString[] {}; 
     chars = new char[] { "W", "h", "o", " ", "a", "r", "e", " ", "y", "o", "u", "?"}; 
     for ( int i = 0; i < chars.length; i++) 
     { 
...
*/    
  ...
  public void testToCharArray()
  {
    ...
    char[] answer = { 'T','h','e',' ','A','u','s','t','r','a','l','i','a','n',' ','P','a','v','i','l','i','o','n','.' };
    for (int i=0; i<result.length; i++) 
      equal(result[i],answer[i]);
    //deepEqual(result, answer); // TODO: deepEquals seems to be broken for char[]
  }
